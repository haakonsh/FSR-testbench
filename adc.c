...
void adc_init(nrf_drv_saadc_event_handler_t  event_handler, adc_struct_t *adc_buffer)
{
    ...
    uint16_t foo = sizeof(adc_struct_t) * NUMBER_OF_STATES;
    
    memset(adc_buffer, 0, foo);
...
{   
    APP_ERROR_CHECK(nrf_drv_saadc_buffer_convert((nrf_saadc_value_t *)&(adc_buffer_p[state]), 3));    
}
